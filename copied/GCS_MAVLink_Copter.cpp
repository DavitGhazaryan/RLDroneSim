...
void GCS_MAVLINK_Copter::send_pid_tuning()
{
    ...
    for (uint8_t i=0; i<ARRAY_SIZE(axes); i++) {
        ...
        // Custom message of all velocity errors
        if (axes[i] == PID_TUNING_ACCZ) {
            const AP_PIDInfo &vpx = copter.pos_control->get_vel_NE_pid().get_pid_info_x();
            const AP_PIDInfo &vpy = copter.pos_control->get_vel_NE_pid().get_pid_info_y();
            const AP_PIDInfo &vpz = copter.pos_control->get_vel_U_pid().get_pid_info();
            if (HAVE_PAYLOAD_SPACE(chan, DEBUG_VECT)) {
                mavlink_msg_debug_vect_send(chan,
                                           "VELZPID",
                                            AP_HAL::micros64(),
                                            vpx.error,        // x
                                            vpy.error,        // y
                                            vpz.target-vpz.actual);       // z
            }
        }
    }
}
